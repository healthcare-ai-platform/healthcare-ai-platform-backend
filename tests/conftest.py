...
async def _fake_user():
    return {"tenant_id": SEED_TENANT_ID, "user_id": SEED_USER_ID}
...
async def unit_client():
    """
    Unit client — mocks all DB connections so no real database is required.
    Auth dependency is overridden to inject seed tenant/user IDs.
    """
    app = create_app()
    app.dependency_overrides[get_current_user] = _fake_user
    with (
...
async def anon_unit_client():
    """
    Unit client with NO auth override — used to test that missing/invalid
    tokens are correctly rejected with 403.
    """
    app = create_app()
    with (
        patch("app.db.session.db.connect",             new_callable=AsyncMock),
